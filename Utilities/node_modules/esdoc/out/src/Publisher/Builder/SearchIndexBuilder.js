...
class SearchIndexBuilder extends _DocBuilder2.default {
  ...
  exec(callback) {
    ...
    for (const doc of docs) {
      ...
      let displayText;      
      if (doc.importPath) {
        displayText = `<span>${ doc.name }</span> <span class="search-result-import-path">${ doc.importPath }</span> <span>${ doc.description }</span>`;
        indexText = `${ doc.importPath }~${ doc.name }~${ doc.description }`.toLowerCase();
        if(doc.params) {
          for(var i in doc.params) {
            indexText += `${ doc.params[i].name }~${ doc.params[i].description }`.toLowerCase();
          }
        }
        if(doc.properties) {
          for(var i in doc.properties) {
            indexText += `${ doc.properties[i].name }~${ doc.properties[i].description }`.toLowerCase();            
          }
        }
